...
import Receipt from "./Main/Receipt/Receipt";
...
export default function PaymentDetails(props) {
  ...
  const useWallet = () => {
    ...
    if (res) {
      if (
        parseFloat(userdata.walletBalance) >=
        parseFloat(payDetails.payableAmount)
      ) {
        let data = { ...payDetails };
        data.usedWalletMoney = parseFloat(data.payableAmount).toFixed(
          settings.decimal
        );
        ...
        data.payableAmount = (
          parseFloat(data.payableAmount) - parseFloat(userdata.walletBalance)
      ...
      data.payableAmount = parseFloat(
        (
          parseFloat(data.payableAmount) + parseFloat(data.usedWalletMoney)
        ).toFixed(settings.decimal)
      );
  ...
  const doPayment = (payment_mode) => {
    if (payment_mode == "cash" || payment_mode == "wallet") {
      ...
      curBooking.customer_paid =
        curBooking.status == "NEW"
          ? parseFloat(payDetails.usedWalletMoney).toFixed(settings.decimal)
          : (
      ...
      curBooking.usedWalletMoney = parseFloat(
      ...
      curBooking.cashPaymentAmount =
        curBooking.status == "NEW"
          ...
          : (
              ...
              (parseFloat(payDetails.usedWalletMoney) +
                parseFloat(payDetails.discount))
            ).toFixed(settings.decimal);
