...
export default function ConvertDriver(props) {
  ...
  const [photoName, setPhotoName] = useState("license");
  ...
  const [state, setState] = useState({
    ...
    carType: null,
    bankAccount:
      auth.info && auth.info.profile.bankAccount
        ? auth.info.profile.bankAccount
        : "",
    bankCode:
      auth.info && auth.info.profile.bankCode ? auth.info.profile.bankCode : "",
    bankName:
      auth.info && auth.info.profile.bankName ? auth.info.profile.bankName : "",
    licenseImage: null,
    carPhoto: null,
    deedOfSale: null,
    cr: null,
    ...
    return (
      <ActionSheet ref={actionSheetRef}>
        <TouchableOpacity
          ...
          onPress={() => {
            _pickImage("CAMERA", ImagePicker.launchCameraAsync, photoName);
          }}
        >
  ...
  const _pickImage = async (permissionType, res, check) => {
    var pickFrom = res;
    let permisions;
    ...
    if (status == "granted") {
      ...
      if (!result.cancelled) {
        ...
        if (check === "license") setImages({ ...images, license: result.uri });
        else if (check === "carPhoto")
          setImages({ ...images, carPhoto: result.uri });
        else if (check === "deedOfSale")
          setImages({ ...images, deedOfSale: result.uri });
        else if (check === "cr") setImages({ ...images, cr: result.uri });
        else if (check === "or") setImages({ ...images, or: result.uri });
        else if (check === "loa") setImages({ ...images, loa: result.uri });
        ...
        if (blob) {
          if (check === "license") setState({ ...state, licenseImage: blob });
          else if (check === "carPhoto") setState({ ...state, carPhoto: blob });
          else if (check === "deedOfSale") setState({ ...state, deedOfSale: blob });
          else if (check === "cr") setState({ ...state, cr: blob });
          else if (check === "or") setState({ ...state, or: blob });
          else if (check === "loa") setState({ ...state, loa: blob });
        }
      }
    } else {
  ...
  //upload cancel
  const cancelPhoto = (photoName) => {
    if (photoName === "license") setImages({ ...images, license: "" });
    if (photoName === "carPhoto") setImages({ ...images, carPhoto: "" });
    if (photoName === "deedOfSale") setImages({ ...images, deedOfSale: "" });
    if (photoName === "cr") setImages({ ...images, cr: "" });
    if (photoName === "or") setImages({ ...images, or: "" });
    if (photoName === "loa") setImages({ ...images, loa: "" });
  };
