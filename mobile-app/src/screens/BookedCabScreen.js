...
export default function BookedCabScreen(props) {
  ...
  useEffect(() => {
    if (activeBookings && activeBookings.length >= 1) {
      ...
      if (booking) {
        ...
        if (
          booking.status == "NEW" &&
          (booking.bookLater == false || (booking.bookLater && diffMins <= 15))
        ) {
          ...
          if (
            ...
            setTimeout(() => {
              ...
              props.navigation.navigate("PaymentDetails", {
                booking: { ...booking, ...booking.selectedBid },
              });
            }, 2000);
          }
        }
  ...
  const renderButtons = () => {
    return (curBooking &&
      role == "rider" &&
      (curBooking.status == "NEW" || curBooking.status == "ACCEPTED")) ||
      (curBooking &&
        role == "driver" &&
        (curBooking.status == "ACCEPTED" ||
  ...
  const startNavigation = () => {
    ...
    if (curBooking.status == "ACCEPTED") {
      ...
      url =
        url + "&destination=" + curBooking.drop.lat + "," + curBooking.drop.lng;
      Linking.openURL(url);
    } else {
                    ...
                    >
                      ...
                      >
                        <Text style={{ color: colors.BLACK, fontSize: 16 }}>
                          {curBooking.driverOffers[key].driver_name}
                        </Text>
                        <Text
                          style={{
                            color: colors.BLACK,
                        ...
                        >
                          {settings.symbol}{" "}
                          {curBooking.driverOffers[key].trip_cost}
                        </Text>
                        <Text style={{ color: colors.BLACK, fontSize: 12 }}>
                          {moment(
                            curBooking.driverOffers[key].deliveryDate
                          ).format("lll")}
                        </Text>
                      </View>
                      <View
                      ...
                      >
                        <Button
                          ...
                          onPress={() =>
                            acceptBid(curBooking.driverOffers[key])
                          }
                          buttonStyle={{
                            width: 90,
  ...
  return (
    <View style={styles.mainContainer}>
      <View style={styles.mapcontainer}>
        ...
        <View
          style={[
            isRTL ? styles.topTitle1 : styles.topTitle,
            { height: settings && settings.otp_secure ? 55 : 45 },
          ]}
        >
        ...
        {role == "driver" && appcat == "delivery" ? (
          <TouchableOpacity
            style={[
              styles.floatButton,
              isRTL ? { left: 10, bottom: 220 } : { right: 10, bottom: 220 },
            ]}
            onPress={() => setPurchaseInfoModalStatus(true)}
          >
            <Icon name="cube" type="ionicon" size={30} color={colors.WHITE} />
          </TouchableOpacity>
        ) : null}
        {role == "driver" ? (
          <TouchableOpacity
            style={[
              styles.floatButton,
              isRTL ? { left: 10, bottom: 150 } : { right: 10, bottom: 150 },
            ]}
        ...
        {curBooking && !(curBooking.status == "NEW") ? (
          <TouchableOpacity
            style={[
              styles.floatButton,
              isRTL ? { left: 10, bottom: 80 } : { right: 10, bottom: 80 },
            ]}
            onPress={() => chat()}
          >
            <Icon
        ...
        {curBooking && !(curBooking.status == "NEW") ? (
          <TouchableOpacity
            style={[
              styles.floatButton,
              isRTL ? { left: 10, bottom: 10 } : { right: 10, bottom: 10 },
            ]}
            onPress={() =>
              role == "rider"
                ? onPressCall(curBooking.driver_contact)
                : appcat == "taxi" || appcat == "rentals"
      ...
      >
         <View style={styles.cabDetailsContainer}>
          {curBooking &&
          curBooking.status == "NEW" &&
          (curBooking.bookLater == false ||
            (curBooking.bookLater &&
              (new Date(curBooking.tripdate) - new Date()) / (1000 * 60) <=
                15)) ? (
            <View
              style={{
                flex: 1,
            ...
            >
              ...
              >
                <Text style={{ fontSize: 22 }}>
                  {curBooking.driverOffers ? t("selectBid") : t("searching")}
                </Text>
              </TouchableOpacity>
            </View>
          ) : null}
          ...
          curBooking.status == "NEW" &&
          curBooking.bookLater &&
          (new Date(curBooking.tripdate) - new Date()) / (1000 * 60) > 15 ? (
            <View
              style={{
                flex: 1,
                width: width,
